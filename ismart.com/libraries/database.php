<?php
function get_brand($parent_id){
    $item = db_fetch_array("SELECT * FROM `tbl_produc_cat` WHERE `parent_id` = $parent_id");
    return $item;
}

function get_info_product_by_code($field, $product_id){
    $info_product_id = db_fetch_row("SELECT `$field` FROM `tbl_products` WHERE `product_code` = '{$product_id}'");
