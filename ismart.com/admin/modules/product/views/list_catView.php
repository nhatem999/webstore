...
<?php get_header();
    $data_post_cat = db_fetch_array('SELECT* FROM `tbl_produc_cat`');
    $list_post_cat_all = data_tree($data_post_cat, 0);
    // $cat_id = db_num_rows("SELECT `parent_id` FROM `tbl_post_cat`");
    $num_per_page = 10;
    # Tổng số bản ghi
    $total_row =  db_num_rows("SELECT * FROM `tbl_produc_cat` ");
    # So trang 
        ...
        <div id="content" class="fl-right">
            ...
            <div class="section" id="paging-wp">
                <div class="section-detail clearfix">
                    ...
                    <?php echo get_pagging($num_page,$pages,"?mod=product&controller=index&action=list_cat") ?>
                </div>
            </div>
        </div>
    </div>
